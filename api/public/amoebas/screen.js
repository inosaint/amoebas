...
const skillUrl = `${location.origin}/amoebas/skill.md`;
...
function drawGrid(viewW, viewH, offsetX, offsetY, mapW, mapH) {
  ...
  for (let x = 0; x <= world.width; x += spacing) {
    ...
    ctx.beginPath(); ctx.moveTo(sx, offsetY); ctx.lineTo(sx, offsetY + mapH); ctx.stroke();
  }
  for (let y = 0; y <= world.height; y += spacing) {
    const sy = offsetY + y * scaleY;
    ctx.beginPath(); ctx.moveTo(offsetX, sy); ctx.lineTo(offsetX + mapW, sy); ctx.stroke();
  }
}
