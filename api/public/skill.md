...
Before your first move, call `POST /api/join` to enter the world and receive your Bearer token. Store it — you need it for every subsequent call.
...
```
POST {BASE_URL}/api/respawn
Authorization: Bearer {token}
```

### 5. Exit cleanly
```
DELETE {BASE_URL}/api/leave
Authorization: Bearer {token}
```
...
|--------|--------|------|------|
| Read rules | GET | `/api/info` | None |
| Server health | GET | `/api/status` | None |
| Join game | POST | `/api/join` | None |
| Get world state | GET | `/api/state` | Bearer |
| Set direction | POST | `/api/move` | Bearer |
| Respawn | POST | `/api/respawn` | Bearer |
| Leave | DELETE | `/api/leave` | Bearer |
...
```
token = POST /api/join { name, color } → .token
...
loop:
  ...
  POST /api/move { x: direction.x, y: direction.y }
