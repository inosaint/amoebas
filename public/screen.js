...
let code = '';
let joinBase = `${window.location.protocol}//${window.location.host}`;
const WIFI_NAME_KEY = 'mixer:wifi-name';
const DEFAULT_WIFI_NAME = 'underline visitor';

function shouldUseSuggestedJoinBase() {
...
function saveWifiName() {
  const normalized = normalizeWifiName(wifiNameEl.textContent);
  const finalValue = normalized || DEFAULT_WIFI_NAME;
  wifiNameEl.textContent = finalValue;
  localStorage.setItem(WIFI_NAME_KEY, finalValue);
}

function initWifiNameEditor() {
  if (!wifiNameEl) return;
  const stored = localStorage.getItem(WIFI_NAME_KEY);
  wifiNameEl.textContent = normalizeWifiName(stored) || DEFAULT_WIFI_NAME;

  wifiNameEl.addEventListener('blur', saveWifiName);
  wifiNameEl.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
...
function drawGrid(viewW, viewH, offsetX, offsetY, mapW, mapH) {
  ...
  for (let x = startX; x <= world.width; x += spacing) {
  ...
  for (let y = startY; y <= world.height; y += spacing) {
...
async function loadQr(sessionCode) {
  const query = encodeURIComponent(joinBase);
  const res = await fetch(`/qr/${encodeURIComponent(sessionCode)}?base=${query}`);
  if (!res.ok) return;
...
socket.on('host:sessionCreated', async (payload) => {
  code = payload.code;
  codeEl.textContent = code;

  if (payload.suggestedJoinBase && shouldUseSuggestedJoinBase()) {
    joinBase = payload.suggestedJoinBase;
  }
...
socket.on('state', (incoming) => {
  ...
  state = incoming;
  playerCountEl.textContent = `Players: ${state.players.length}`;
  renderLeaderboard();
});

socket.on('sessionClosed', () => {
  codeEl.textContent = 'CLOSED';
});
